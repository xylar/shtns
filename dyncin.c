...
struct VectField *U,*B,*J;		// access by : B[ir].r[lm]
struct PolTor *Ulm, *Blm, *NLb1, *NLb2, *NLu1, *NLu2;
*/

// Magnetic field representation.
double **Br, **Bt, **Bp;	// Spatial representation
complex double **BPlm, **BTlm;	// Spherical Harmonics representation
complex double **NLbp1, **NLbp2, **NLbt1, **NLbt2;	// Adams-Bashforth : 2 non-linear terms stored.
...
void alloc_Bfields()
{
	...
	Br = (double **) malloc( 3*NR* sizeof( double **) );
	Bt = Br + NR;
	Bp = Br + 2*NR;
	BPlm = (complex double **) malloc( 6*NR* sizeof( complex double *) );
	BTlm = BPlm + NR;
	NLbp1 = BPlm + 2*NR;
	NLbp2 = BPlm + 3*NR;
	NLbt1 = BPlm + 4*NR;
...
int main (int argc, char *argv[])
{
	...
	if (argc < 2) runerr("no Rm given on command line !");
